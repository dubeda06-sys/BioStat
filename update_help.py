...
def update_file(filepath):
    ...
    # Parse and replace ANALYSIS_HELP
    import ast
    
    # Simple search for ANALYSIS_HELP = { ... }
    # We will use regex to find the start and end of ANALYSIS_HELP
    
    match_help = re.search(r'ANALYSIS_HELP\s*=\s*\{.*?\n\}', content, re.DOTALL)
    if not match_help:
        print("ANALYSIS_HELP no encontrado.")
        return
        
    # Construir nueva cadena
    new_dict_str = "ANALYSIS_HELP = {\n"
    for k, v in EXPANDED_HELP.items():
        # Escapar comillas dobles
        v_escaped = v.replace('"', '\\"')
        new_dict_str += f'    "{k}": "{v_escaped}",\n'
    new_dict_str += "}"
    
    content = content.replace(match_help.group(0), new_dict_str)
    
    # También actualizar ANALYSIS_LEGENDS para que coincida el legend
    match_leg = re.search(r'ANALYSIS_LEGENDS\s*=\s*\{.*?\n\}\n', content, re.DOTALL)
    if match_leg:
        # Reemplazar los textos de 'legend' dentro de ANALYSIS_LEGENDS
        for k, v in EXPANDED_HELP.items():
            pattern = r'("' + re.escape(k) + r'"\s*:\s*\{\s*"legend"\s*:\s*)"([^"]*)"'
            content = re.sub(pattern, r'\g<1>"' + v.replace('"', '\\"') + '"', content)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    
    print("Glosas de ayuda actualizadas exitosamente.")

if __name__ == "__main__":
    import sys
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'src', 'ui', 'analysis_panel.py')
    update_file(path)
